...
document
  .querySelector(".todos")
  .addEventListener("click", () => cardProduto(produtosTenis));

document
  .querySelector(".acessorios")
  .addEventListener("click", filtrarAcessorios);
function filtrarAcessorios() {
  const acessorio = produtosTenis.filter(
    (produto) => produto.tag[0] === "Acessórios"
  );
...
function filtrarRaquetes() {
  const raquetes = produtosTenis.filter(
    (produto) => produto.tag[0] === "Raquetes"
  );
...
document
  .querySelector(".produtos__carrinho")
  .addEventListener("click", removerCarrinho);
...
function pesquisarProduto(event) {
  ...
  if (pesquisa === "") {
    ...
    const produtosFiltrados = produtosTenis.filter((produto) =>
      produto.nameItem
        .toLocaleLowerCase()
